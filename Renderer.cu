// See https://stackoverflow.com/questions/13893919/cuda-syncthreads-compiles-fine-but-is-underlined-with-red
// You don't actually need these code to include device_functions for the renderer to compile
// But the intellisense somehow messes up about __syncthreads and other builtins
#ifndef __CUDACC__
#define __CUDACC__
#endif
...
__device__ unsigned long int d_getChildCountFromMask(unsigned long int mask)
{
    return (1ul & mask) +
           ((2ul & mask) >> 1) +
           ((4ul & mask) >> 2) +
...
Renderer::Renderer(int frameWidthInPixels, int frameHeightInPixels, bool shadowMapping) : m_frameWidth(frameWidthInPixels),
