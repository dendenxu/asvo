...
Glue::Glue(int argc, char **argv, int windowWidth, int windowHeight,
           ...
           bool &outSuccess)
    : m_windowWidth(windowWidth), m_windowHeight(windowHeight), m_renderer(renderer), m_model(model), m_light(light), m_camera(camera), m_lastFrameTimeInMilliseconds(0)
{
...
void Glue::display()
{
    ...
    m_renderer->render(*m_model, m_camera, m_light, animationFrameIndex,
